...
public class ChattyApplication {
	...
	CommandLineRunner runner(UserRepository userRepository, MongoTemplate mongoTemplate) { // Executes a command
		...
		return args -> {
			...
			// insertStudentMongoTemplateAndQuery(userRepository, mongoTemplate);
			
			userRepository.findByEmail("email@email.com").ifPresentOrElse( user -> {
				System.out.println("This user was found --> " + user);
			}, () -> {
